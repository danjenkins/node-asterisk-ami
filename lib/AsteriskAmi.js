...
AsteriskAmi.prototype.connect = function(){
  ...
  carrier.carry(self.connection, function carrierCb(rawEvent) {
    ...
    if(event.actionid && self._callbacks.hasOwnProperty(event.actionid) && self._callbacks[event.actionid] instanceof Function){
      self._callbacks[event.actionid](null, event);
      //delete self._callbacks[event.actionid];
      //cant delete as more than one response can come back per actionid
      //need to think of how to do it
    }
    ...
    if(event && event.event && typeof event.event == 'string' && self.listeners(event.event.toLowerCase()).length){
      self.emit(event.event.toLowerCase(), event);
    }

    if(self.listeners('data').length && Object.keys(event).length){
...
AsteriskAmi.prototype._send = function(obj, cb){
  ...
  this.connection.write(stringify.call(this, obj), this.amiEncoding);
};
